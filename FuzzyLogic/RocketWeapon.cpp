...
RocketWeapon::RocketWeapon(const int roundLeft_)
	: roundLeft(roundLeft_)
{
	InitializeFuzzyModule();
}

void RocketWeapon::InitializeFuzzyModule()
{
	...
	fm.AddRule(targetMediumAmmoLoads, veryDesirable);
	fm.AddRule(targetMediumAmmoOkay, veryDesirable);
	fm.AddRule(targetMediumAmmoLow, desirable);
	fm.AddRule(targetFarAmmoLoads, desirable);
	fm.AddRule(targetFarAmmoOkay, undesirable);
	fm.AddRule(targetFarAmmoLow, undesirable);
}

double RocketWeapon::GetDesirability(double distToTarget)
{
